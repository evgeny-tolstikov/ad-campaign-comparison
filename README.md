...
## Саммари
Сравнение 2 каналов привлечения (YouTube-интеграция vs таргет в соцсетях) по CAC, ARPPU, AOV, ROI и retention на данных учебной БД PostgreSQL. Пайплайн на Python генерирует SQL-запрос из списков user_id и собирает финальный отчёт с графиками. В итоге YouTube-интеграция окупилась на 5-й день при более высоком CAC, но большем числе заказов на пользователя, таргет пока в минусе. Рекомендация – собрать данные за 14 или 28 дней.
